...
# dev 한정. Phase 1에 Keycloak 프로덕션 모드 + 강화된 SSO로 복원.
...
TOKEN=$($CURL \
  ...
  -d "client_secret=ocr-backoffice-dev-secret" \
  -d "username=dev-admin" \
  -d "password=dev-admin-pw" \
...
ACTIVE=$($CURL \
  -u "ocr-backoffice:ocr-backoffice-dev-secret" \
  -d "token=$TOKEN" \
